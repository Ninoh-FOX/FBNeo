...
int (*GetNextSound)(int);               // Callback para solicitar más sonido
...
static int SDLSoundGetNextSoundFiller(int) {
    if (nAudNextSound == NULL) return 1;
    memset(nAudNextSound, 0, nAudSegLen << 2); // Escribe silencio en el buffer
    return 0;
}
...
static int SDLSoundCheck() {
    ...
    if (!bAudPlaying) return 1;

    nPlaySeg = nSDLPlayPos / (nAudSegLen << 2) - 1;
    if (nPlaySeg >= nAudSegCount) nPlaySeg -= nAudSegCount;
    if (nPlaySeg < 0) nPlaySeg = nAudSegCount - 1;

    if (nSDLFillSeg == nPlaySeg) return 0;

    nFollowingSeg = nSDLFillSeg;
    WRAP_INC(nFollowingSeg);

    while (nSDLFillSeg != nPlaySeg) {
        int bDraw = (nFollowingSeg == nPlaySeg);
        GetNextSound(bDraw);
...
static int SDLSetCallback(int (*pCallback)(int)) {
    GetNextSound = pCallback ? pCallback : SDLSoundGetNextSoundFiller;
    return 0;
}
